...
public:
    ...
    void Draw(Shader &shader)
    {
        for(unsigned int i = 0; i < meshes.size(); i++){
            shader.setMat4("model", getModelMatrix(meshes[i].name));
    ...
    void setTransform(const string&  hierarchy_name, glm::mat4 &transformation)
    {
...
private:
    ...
    void printHierarchy(const string& name){
        ...
        {
            printHierarchy(child->hierarchy_name);
        }
    }
    //* -- END OF HEIRARCHY FUNCTIONS
