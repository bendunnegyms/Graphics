...
#define MESH_NAME "PenguinBaseMesh.obj"
...
void display() {
	...
	mview = translate(identity_mat4(), vec3(0.0, 0.0, -10.0));
	mat4 persp_proj = perspective(45.0, (float)width / (float)height, 0.1, 100.0);
	mat4 model = rotate_z_deg(identity_mat4(), 0);
	model = rotate_x_deg(identity_mat4(), 0);
	model = rotate_y_deg(identity_mat4(), 0);

	glViewport(0, 0, width, height);
	glUniformMatrix4fv(proj_mat_location, 1, GL_FALSE, persp_proj.m);
	glUniformMatrix4fv(view_mat_location, 1, GL_FALSE, mview.m);
	glUniformMatrix4fv(matrix_location, 1, GL_FALSE, model.m);
	//Camera
	
	// Set up the child matrix
	mat4 modelChild = identity_mat4();
	modelChild = rotate_z_deg(modelChild, rotate_z);
	modelChild = rotate_y_deg(modelChild, rotate_y);
	modelChild = rotate_x_deg(modelChild, rotate_x);
	modelChild = translate(modelChild, vector_1);
...
void keypress(unsigned char key, int x, int y) {
	if (key == 'w') {
		vector_1 += vec3(0.0f, -0.05f, 0.0f);
	}

	if (key == 'a')
	{
		vector_1 += vec3(0.05f, 0.0f, 0.0f);
	}
	if (key == 's')
	{
		vector_1 += vec3(0.0f, 0.05f, 0.0f);
